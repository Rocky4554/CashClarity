...
import { useDispatch, useSelector } from "react-redux";
import { updateActualCount } from "@/reduxStore/userSlice";
import { ProFeatureDialog } from "@/components/dialogBox/proDialogBox/pro-feature-dialog";
import { ProFeatureWrapper } from "@/components/dialogBox/proDialogBox/pro-feature-wrapper";
...
export function AddTransactionForm({
  ...
  // Add ref to track if we've already processed the success
  const processedTransactionRef = useRef(null);
  ...
  const onSubmit = (data) => {
    ...
    setIsSubmitting(true);
    // Reset the processed transaction ref when starting a new submission
  ...
  useEffect(() => {
    // Check if we have a successful result and we haven't processed it yet
    if (
      transactionResult?.success && 
      !transactionLoading && 
      processedTransactionRef.current !== transactionResult.data?.id
    ) {
      ...
      setIsSubmitting(false);
    } else if (transactionResult?.error && processedTransactionRef.current !== 'error') {
      // Handle error case and prevent multiple error toasts
      processedTransactionRef.current = 'error';
      toast.error(transactionResult.error || "Failed to process transaction");
      setIsSubmitting(false);
    }
  }, [transactionResult?.success, transactionResult?.data?.id, transactionLoading]);

  // Separate useEffect for loading state management
  useEffect(() => {
    if (!transactionLoading && isSubmitting) {
      setIsSubmitting(false);
    }

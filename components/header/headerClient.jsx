...
// export default function HeaderClient({ accounts = [] ,user}) {
...
//   return (
//     <header className={`fixed top-0 w-full ${user.isPro ? 'bg-[#f4f6f7]' : 'bg-white/80'} backdrop-blur-md z-50 border-b`}>
...
//             >
//               <Button variant="outline" className="flex items-center gap-2 shadow-md">
//                 <BadgeIndianRupee size={18} className="text-blue-500 animate-pulse" />
//                 <span className="hidden md:inline text-blue-500 font-semibold">Transactions</span>
...
//                     ))
//                   ) : (
//                     <div className="px-4 py-2 text-sm text-gray-500">No accounts</div>
//                   )}
//                 </div>
//               )}
//             </div>

//             {/* Add Transaction Button - black bg, white text, shimmer border, shadow */}
//             <Link href="/transaction/create">
//               <Button
//                 variant="outline"
//                 className="relative flex items-center gap-2 bg-black text-white shadow-md border border-white overflow-hidden"
//               >
//                 <PenBox size={18} className="z-10" />
//                 <span className="hidden md:inline z-10">Add Transaction</span>
//                 <span className="absolute inset-0 border-2 border-transparent rounded animate-shimmer-border pointer-events-none" />
//               </Button>
//             </Link>

//             {/* Upgrade Pro Button - golden, shimmer, always visible text */}
...
//   return (
//     <header
//       className={`fixed top-0 w-full ${isPro ? "bg-gradient-to-r from-blue-50/90 to-purple-50/90 border-blue-200/50" : "bg-white/80 border-gray-200"} backdrop-blur-md z-50 border-b transition-all duration-500`}
//     >
//       <nav className="container mx-auto px-4 py-4 flex items-center justify-between">
//         {/* Logo */}
//         <Link href="/">
...
//               <Button
//                 variant="outline"
//                 className="relative flex items-center gap-2 shadow-md overflow-hidden bg-black text-white border-white hover:bg-gray-800"
//               >
//                 <PenBox size={18} className="z-10" />
//                 <span className="hidden md:inline z-10">Add Transaction</span>
...
// export default function HeaderClient() {
//   const { isSignedIn, user, isLoaded } = useUser(); // Clerk's client-side hook
//   const refreshUser = useRefreshUser();
...
//   // ✅ FIX 1: Use accounts.length instead of the accounts array itself
//   // This prevents the dependency array from changing size/type
//   useEffect(() => {
//     if (isLoaded && isSignedIn) {
//       refreshUser(); // Only refresh when user is actually signed in
//     }
//   }, [isLoaded, isSignedIn, refreshUser, accounts?.length]); // Use primitive values only

//   // Alternative fix - if you need to trigger on account changes, use useMemo
//   // const accountIds = useMemo(() => 
//   //   accounts?.map(acc => acc.id).join(',') || '', 
//   //   [accounts]
//   // );
//   // 
//   // useEffect(() => {
...
//   //     refreshUser();
//   //   }
//   // }, [isLoaded, isSignedIn, refreshUser, accountIds]);
...
//       );
//     }
//   }, [accounts?.length]); // ✅ FIX 2: Use accounts.length instead of accounts
